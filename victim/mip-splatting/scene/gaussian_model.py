...
class GaussianModel:
    ...
    def load_ply(self, path):
        ...
        if "opacity" in plydata.elements[0].data.dtype.names:
            ...
            print("Warning: No opacity field found in ply file, using default value 1.0")
            opacities = np.ones((xyz.shape[0], 1), dtype=np.float32)

        # 检查是否有 filter_3D 字段，如果没有则创建默认值
        if "filter_3D" in plydata.elements[0].data.dtype.names:
            filter_3D = np.asarray(plydata.elements[0]["filter_3D"])[..., np.newaxis]
        else:
            print("Warning: No filter_3D field found in ply file, creating default values")
